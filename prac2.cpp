...
double get_score(node *p, string tname)
{
    ...
    if (p->name == tname)
        return p->score; // Found the node with the matching name, return its score

    // Recursively search in the left and right subtrees
    double left_score = get_score(p->left, tname);
    if (left_score != -1) // If found in the left subtree, return the score
        return left_score;

    double right_score = get_score(p->right, tname);
    if (right_score != -1) // If found in the right subtree, return the score
        return right_score;

    return -1; // If not found in both subtrees, return -1
}
